...
│   │  定时任务 (APScheduler)                               │   │
│   │                                                     │   │
│   │  当前：提醒检查 / 主动消息机会（按实现启用）              │   │
│   │  规划：记忆衰减 / 候选合并 / 浮现缓存 / 日记草稿          │   │
│   └─────────────────────────────────────────────────────┘   │
│                                                             │
│   ┌─────────────────────────────────────────────────────┐   │
│   │  MCP Server (FastMCP)                                │   │
...
| 技术 | 用途 |
|---|---|
| **APScheduler 3.x** | 当前用于已实现的后台任务；记忆衰减、夜间摘要、浮现缓存、日记草稿仍是规划项 |
...
- 单用户产品完全够用
- 零部署成本，不需要额外安装数据库服务
- 可以用 SQLite `.backup` 生成单文件一致快照，备份简单
- 迁移方便——用一致快照搬库，不需要额外数据库服务
...
当前处于记忆升级 Phase A。首发用关键词 + tag 匹配实现 recall 和关联，架构上预留向量字段（`embedding BLOB`），Phase B 再切换到 embedding。

当前已落地的四层记忆：

| layer | 用途 | 当前状态 |
|---|---|---|
| `core` | 核心记忆，不衰减 | 字段/API 已实现 |
| `long` | 长期记忆，慢慢变淡 | 字段/API 已实现 |
| `short` | 短期记忆，较快过期 | 字段/API 已实现 |
| `consciousness` | Connie 内部行动备忘 | 字段/API 已实现 |

当前聊天链路：
- Remoire 前端每轮聊天自动调用 `recall(user_message, limit=5)`，把结果注入 system prompt。
- 召回为空时注入防编造提示。
- `resume()` 当前主要用于 Claude.ai MCP，不是前端每轮聊天自动注入。
- 聊天结束后后台异步提取记忆候选，高置信度自动入库，低置信度进入候选区。
...
| **数据备份** | cron 定时用 `sqlite3 ".backup"` 生成 SQLite 一致快照 |
...
5. **定期备份** — 每天用 SQLite `.backup` 自动生成一致快照，可选下载到自己电脑
