"""
Remoire Remote MCP Server

Streamable HTTP transport for Claude web/mobile custom connectors.
Local endpoint: http://127.0.0.1:8001/mcp
Tunnel endpoint: https://<name>.trycloudflare.com/mcp
"""
import os
import sys
...
mcp = FastMCP(
    ...
    host="127.0.0.1",
    port=int(os.getenv("MCP_REMOTE_PORT", "8001")),
    ...
    transport_security=TransportSecuritySettings(
        ...
        allowed_hosts=[
            "127.0.0.1:*",
            "localhost:*",
            "dvds-veterans-miss-conditions.trycloudflare.com",
            "*.trycloudflare.com",
        ],
        allowed_origins=[
            "http://127.0.0.1:*",
...
async def resume() -> str:
    """醒来时调用。返回最近的记忆、未读纸条、最近日记，帮助你恢复对静儿的感知。"""
    ...
    if memories:
        parts.append("【最近的记忆】")
        parts.extend(f"- {m['content']}" for m in memories[:10])
    ...
    if diaries:
        ...
        for diary in diaries:
            parts.append(f"- 【{diary['title']}】{diary['content'][:80]}...")

    return "\n".join(parts) if parts else "记忆库还是空的，等待和静儿创造新的回忆。"
...
async def read_diary(limit: int = 5) -> str:
    ...
    if not diaries:
        return "日记本里还没有内容。"
    return "\n\n---\n\n".join(
        f"【{diary['title']}】({diary['created_at'][:10]})\n{diary['content']}"
        for diary in diaries
    )
